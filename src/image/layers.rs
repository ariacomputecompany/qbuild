...
impl LayerManager {
    ...
    pub async fn extract_layer(&self, digest: &str, blob: &[u8]) -> Result<PathBuf> {
        ...
        // Reuse only verified extracted layers. Older corrupt cache directories
        // are removed and rebuilt from the canonical blob.
        if layer_path.exists() {
            if self.layer_is_ready(&layer_path) {
        ...
        let temp_path = self.unique_temp_layer_path(hash);
        ...
        self.mark_layer_ready(&temp_path)?;

        // Atomic finalize. If another extractor won the race and published a
        // verified layer first, discard this temp directory and reuse it.
        self.finalize_extracted_layer(&temp_path, &layer_path)?;
        ...
        for entry in archive.entries().map_err(|e| {
            ...
            match entry_type {
                tar::EntryType::Directory => {
                    self.remove_conflicting_path(&full_path, true)?;
                ...
                tar::EntryType::Regular | tar::EntryType::Continuous => {
                    self.remove_conflicting_path(&full_path, false)?;
                ...
                tar::EntryType::Link => {
                    if let Ok(Some(target)) = entry.link_name() {
                        ...
                        self.remove_conflicting_path(&full_path, false)?;
                        std::fs::hard_link(&target_path, &full_path).map_err(|e| {
                            ImageError::LayerExtractionError(format!(
    ...
    fn layer_ready_marker(&self, path: &Path) -> PathBuf {
        path.join(".quilt-layer-ready")
    }

    fn layer_is_ready(&self, path: &Path) -> bool {
        path.is_dir() && self.layer_ready_marker(path).is_file()
    }

    fn mark_layer_ready(&self, path: &Path) -> Result<()> {
        std::fs::write(self.layer_ready_marker(path), b"v1").map_err(ImageError::from)
    }

    fn unique_temp_layer_path(&self, hash: &str) -> PathBuf {
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_nanos())
            .unwrap_or_default();
        self.layers_path
            .join(format!("{}.tmp.{}.{}", hash, std::process::id(), nanos))
    }

    fn finalize_extracted_layer(&self, temp_path: &Path, layer_path: &Path) -> Result<()> {
        match std::fs::rename(temp_path, layer_path) {
            Ok(()) => Ok(()),
            Err(err) => {
                if layer_path.exists() && self.layer_is_ready(layer_path) {
                    let _ = std::fs::remove_dir_all(temp_path);
                    return Ok(());
                }
                ...
                let _ = std::fs::remove_dir_all(temp_path);
                Err(ImageError::LayerExtractionError(format!(
                    "Failed to finalize extracted layer {}: {}",
                    layer_path.display(),
                    err
                )))
            }
    ...
    fn remove_conflicting_path(&self, path: &Path, expect_directory: bool) -> Result<()> {
        let metadata = match std::fs::symlink_metadata(path) {
            Ok(metadata) => metadata,
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => return Ok(()),
            Err(err) => {
                return Err(ImageError::LayerExtractionError(format!(
        ...
        if metadata.is_dir() {
            std::fs::remove_dir_all(path).map_err(|e| {
                ImageError::LayerExtractionError(format!(
                    "Failed to remove conflicting directory {}: {}",
                    path.display(),
                    e
                ))
            })?;
            ...
            std::fs::remove_file(path).map_err(|e| {
                ImageError::LayerExtractionError(format!(
                    "Failed to remove conflicting path {}: {}",
                    path.display(),
                    e
                ))
            })?;
...
mod tests {
    ...
    #[test]
    fn test_layer_reextracts_without_ready_marker() {
        let dir = tempdir().unwrap();
        let manager = LayerManager::new(dir.path()).unwrap();
        ...
        let mut tar_data = Vec::new();
        {
            ...
            let data = b"healthy";
            let mut header = tar::Header::new_gnu();
            header.set_path("usr/bin/dash").unwrap();
            header.set_size(data.len() as u64);
        ...
        let digest = "sha256:repair_layer";
        let hash = digest.split(':').nth(1).unwrap();
        let layer_path = dir.path().join("layers").join(hash);
        std::fs::create_dir_all(layer_path.join("usr/bin")).unwrap();
        std::fs::write(layer_path.join("usr/bin/dash"), b"").unwrap();

        let rt = tokio::runtime::Runtime::new().unwrap();
        let extracted = rt
            .block_on(manager.extract_layer(digest, &tar_data))
            .unwrap();
        assert_eq!(
            std::fs::read(extracted.join("usr/bin/dash")).unwrap(),
            b"healthy"
        );
        assert!(extracted.join(".quilt-layer-ready").exists());
    }

    #[test]
    fn test_layer_extraction_ignores_stale_shared_temp_dir_name() {
        let dir = tempdir().unwrap();
        let manager = LayerManager::new(dir.path()).unwrap();
        let digest = "sha256:stale-temp-dir";
        let hash = digest.split(':').nth(1).unwrap();

        std::fs::create_dir_all(dir.path().join("layers").join(format!("{}.tmp", hash))).unwrap();
        ...
        let mut tar_data = Vec::new();
        {
            ...
            let mut header = tar::Header::new_gnu();
            header
                .set_path("usr/share/zoneinfo/right/America/Barbados")
                .unwrap();
            header.set_size(data.len() as u64);
            header.set_mode(0o644);
            header.set_cksum();
        ...
        let extracted = rt
            ...
            .unwrap();
        assert_eq!(
            std::fs::read(extracted.join("usr/share/zoneinfo/right/America/Barbados")).unwrap(),
            b"ok"
        );
    }

    #[test]
    fn test_layer_extraction_replaces_conflicting_symlink_with_directory() {
        let dir = tempdir().unwrap();
        let manager = LayerManager::new(dir.path()).unwrap();
        ...
        let mut tar_data = Vec::new();
        {
            ...
            symlink
                .set_path("usr/share/zoneinfo/right/America")
                .unwrap();
            symlink.set_link_name("../America").unwrap();
            symlink.set_size(0);
            ...
            file.set_path("usr/share/zoneinfo/right/America/Barbados")
                .unwrap();
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let extracted = rt
            .block_on(manager.extract_layer("sha256:path-conflict", &tar_data))
            .unwrap();
        assert_eq!(
            std::fs::read(extracted.join("usr/share/zoneinfo/right/America/Barbados")).unwrap(),
            b"barbados"
    ...
    #[test]
    fn test_layer_extraction_handles_concurrent_same_digest_requests() {
        let dir = tempdir().unwrap();
        let manager = Arc::new(LayerManager::new(dir.path()).unwrap());
        ...
        let paths = rt.block_on(async {
            ...
            for _ in 0..4 {
                ...
                let blob = tar_data.clone();
                tasks.push(tokio::spawn(async move {
                    manager
                        .extract_layer("sha256:concurrent-layer", &blob)
                        .await
                        .unwrap()
                }));
            }
